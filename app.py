...
# Helper function to get quadrant name based on coordinates
...
market_choice = st.sidebar.selectbox(
    ...
    options=[
        "Indian NSE Sectoral Indices",
        "Nifty 50 Heavyweights",
        "Nifty 100 Leaders",
        "Nifty 500 Momentum Stars",
        "US Equities (S&P 500)",
        "Crypto Currencies",
        "Custom List"
    ],
...
# Define robust preset mappings
benchmark_default = "^NSEI"
tickers_default = []

if market_choice == "Indian NSE Sectoral Indices":
    benchmark_default = "^NSEI"
    tickers_default = ["^NSEBANK", "^CNXIT", "^CNXMETAL", "^CNXPHARMA", "^CNXAUTO", "^CNXFMCG", "^CNXENERGY", "^CNXINFRA", "^CNXREALTY", "^CNXPSUBANK"]
elif market_choice == "Nifty 50 Heavyweights":
    benchmark_default = "^NSEI"
    tickers_default = ["RELIANCE.NS", "TCS.NS", "HDFCBANK.NS", "INFY.NS", "ICICIBANK.NS", "SBIN.NS", "ITC.NS", "AXISBANK.NS", "LT.NS", "BHARTIENTL.NS"]
elif market_choice == "Nifty 100 Leaders":
    benchmark_default = "^NSEI"
    # Mid-to-Large momentum caps from Nifty 100
    tickers_default = ["TRENT.NS", "HAL.NS", "BEL.NS", "ZOMATO.NS", "JIOFIN.NS", "ADANIENT.NS", "PFC.NS", "RECLTD.NS", "COALINDIA.NS", "NTPC.NS", "VBL.NS", "M&M.NS"]
elif market_choice == "Nifty 500 Momentum Stars":
    benchmark_default = "^NSEI"
    # Select high momentum mid/small growth stocks from Nifty 500
    tickers_default = ["RVNL.NS", "IRFC.NS", "SUZLON.NS", "BHEL.NS", "HUDCO.NS", "HDFCAMC.NS", "DIXON.NS", "MAHABANK.NS", "TATAELXSI.NS", "MCX.NS", "YESBANK.NS", "GMRINFRA.NS"]
...
with tab1:
    ...
    if error:
        ...
        # Plotly shape coordinates are defined strictly using x0, y0, x1, y1 (y2 is invalid)
        ...
        # Axis crosshair lines (x0, y0, x1, y1)
        fig.add_shape(type="line", x0=100.0, y0=min_y, x1=100.0, y1=max_y, line=dict(color="rgba(255,255,255,0.15)", width=1.5, dash="dash"))
        fig.add_shape(type="line", x0=min_x, y0=100.0, x1=max_x, y1=100.0, line=dict(color="rgba(255,255,255,0.15)", width=1.5, dash="dash"))

        # Quadrant labels at extreme corners (showarrow in python must be capitalized as False)
