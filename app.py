...
def calculate_rrg(tickers, benchmark, period="1y", interval="1d", window=14):
    try:
        # Step 1: Download Closing prices for all tickers + benchmark
        all_tickers = tickers + [benchmark]
        data = yf.download(all_tickers, period=period, interval=interval, group_by='ticker', progress=False)
        
        # Parse adjusted close
        prices = pd.DataFrame()
        for ticker in all_tickers:
            if ticker in data:
                prices[ticker] = data[ticker]['Close']
            else:
                # Fallback if download grouping differs
                prices[ticker] = data['Close'][ticker]
                
        # Handle missing data
        prices = prices.ffill().dropna()
        if prices.empty:
            return None, "Downloaded price data is empty. Please verify your tickers."
...
with tab1:
    ...
    if error:
        ...
        # Add Quadrant Background Fills
        # Mapped dimensions: We set standard view range from 95 to 105 for high clarity,
        # but dynamically scale based on data.
        all_x = []
        all_y = []
        ...
        if all_x and all_y:
            min_x = min(98.0, min(all_x) - 0.5)
            max_x = max(102.0, max(all_x) + 0.5)
            min_y = min(98.0, min(all_y) - 0.5)
            max_y = max(102.0, max(all_y) + 0.5)
        else:
            min_x, max_x, min_y, max_y = 95, 105, 95, 105

        # Configure background quadrant annotations/shading
        fig.add_shape(type="rect", x0=100, y0=100, x1=max_x, y1=max_y, fillcolor="rgba(16, 185, 129, 0.03)", line_width=0, layer="below") # Leading
        fig.add_shape(type="rect", x0=100, y0=min_y, x1=max_x, y1=100, fillcolor="rgba(245, 158, 11, 0.03)", line_width=0, layer="below") # Weakening
        fig.add_shape(type="rect", x0=min_x, y0=min_y, x1=100, y1=100, fillcolor="rgba(239, 68, 68, 0.03)", line_width=0, layer="below") # Lagging
        fig.add_shape(type="rect", x0=min_x, y0=100, x1=100, y1=max_y, fillcolor="rgba(59, 130, 246, 0.03)", line_width=0, layer="below") # Improving

        # Axis crosshair lines
        fig.add_shape(type="line", x0=100, y0=min_y, x1=100, y2=max_y, line=dict(color="rgba(255,255,255,0.15)", width=1.5, dash="dash"))
        fig.add_shape(type="line", x0=min_x, y0=100, x1=max_x, y2=100, line=dict(color="rgba(255,255,255,0.15)", width=1.5, dash="dash"))
        ...
        for asset, df in rrg_results.items():
            ...
            elif latest['RS_Ratio'] >= 100 and latest['RS_Momentum'] < 100:
